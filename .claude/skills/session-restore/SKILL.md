...
9. Mode de renumérotation (`renumber_mode = 1`, auto), mode de redressement (`straighten_mode = 0`, manuel) et compression ZIP (niveau 0, stocké) remis par défaut sur tous les panneaux — voir skills `renumbering`/`page-straighten`/`zip-compression`. Contrairement au bloc de renumérotation (qui n'appelle que `_update_status_bar()`), le redressement appelle en plus `_refresh_toolbar_states()` pour rafraîchir immédiatement le tooltip/état visuel du bouton bi-mode de la colonne d'icônes (voir skill `page-straighten`, section bascule manuel/auto) — sans cet appel, le tooltip resterait affiché sur l'ancien mode jusqu'au prochain événement qui déclenche un rafraîchissement.
...
- **Le fichier de config est dans `%APPDATA%\MosaicView\`**, pas dans `%TEMP%\MosaicViewTemp\` (voir skill `config-storage`) — la session sauvegardée (et les marque-pages) n'est donc pas exposée à une purge système du dossier temporaire. Seule l'action "Effacer le fichier de configuration" (explicite, voulue par l'utilisateur) peut faire disparaître la session sauvegardée entre deux lancements ; "Effacer les fichiers temporaires" n'a aucun effet dessus.
