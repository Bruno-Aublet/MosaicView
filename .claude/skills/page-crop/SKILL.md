...
## Bouton "Valider" flottant — partagé avec straighten/texte/formes (v1.7.3+, comportement revu v1.7.5)

**Depuis le 2026-08-15, ce n'est plus un simple hide/show** : le bouton reste TOUJOURS VISIBLE tant que l'outil crop est actif dans la barre (vert/actif dès qu'un rectangle valide existe, gris/inactif sinon), positionné sous la barre d'outils (crop n'a pas de panneau d'options flottant dédié, contrairement à straighten/texte/formes) — comportement introduit d'abord pour l'outil formes (skill `draw-shapes`) puis généralisé aux 3 autres outils sur demande explicite utilisateur. Détail complet de l'architecture (3 fonctions aux responsabilités séparées — `_update_validate_btn_state`/`_reveal_validate_btn`/`_conceal_validate_btn` — et mécanisme unique de réapparition/masquage piloté exclusivement par `_ViewerToolbar`) : skill `viewers`, section "Ce qui reste dans `image_viewer_qt.py`". Ce skill-ci ne documente que ce qui reste spécifique au crop.
