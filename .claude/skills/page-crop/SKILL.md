...
**Depuis v1.7.3, ce n'est plus un bouton dédié au crop** : c'est le même `QPushButton` (sous-classe `_ValidateButton`) réutilisé par straighten/texte/formes, texte et action câblés dynamiquement selon `self._viewer._toolbar.active_tool` (`_VALIDATE_KEYS`, dict au niveau classe de `_ViewerCanvas`). Cliquer dessus appelle `ImageViewer.validate_crop()` si `active_tool == "crop"`. L'état de connexion du signal `clicked` est mémorisé (`_validate_btn_connected_tool`) pour ne reconnecter le signal que lors d'un vrai changement d'outil — reconnecter à chaque mise à jour d'état déclenchait un `RuntimeWarning` de PySide6 au tout premier affichage.
...
**N'a pas de section dédiée dans le mode d'emploi** (skill `user-guide`) : décrit dans `help.viewer_content`, au même titre que les autres outils de la barre — contrairement à `page-resize`, qui a sa propre section.
