...
- **Le préfixe de nommage diffère selon le chemin** : `NEW-` (images isolées ajoutées à une session ouverte, ou archives 2..N d'un chargement multi-fichiers) vs `merge_prefix` unique incrémental (fusion explicite d'une archive via `import_merge_qt.py`) — ne pas mélanger les deux conventions en copiant un pattern de l'un vers l'autre sans vérifier lequel s'applique.
