...
## Colonne d'icônes — mono-fonction automatique (depuis le 2026-08-14)

**L'icône `straighten` de la colonne d'icônes n'est plus bi-mode** — nettoyage effectué une fois le redressement manuel migré dans la barre d'outils de la visionneuse (voir `idees.txt` #3, section "NETTOYAGE DES COMMANDES REDONDANTES") : elle ne déclenche plus que le redressement **automatique en lot** (`deskew_selected_qt(...)` sur la sélection courante de la mosaïque), au même titre que ses homologues du menu Images et du menu contextuel. Le clic droit sur cette icône n'a plus d'effet spécial (plus de bascule de mode) ; son tooltip est désormais fixe (`tooltip.straighten` = "Redresser l'image automatiquement", plus de `tooltip.straighten_mode_0`/`_1` dynamique côté colonne).

Ce qui a disparu de `panel_widget.py` à cette occasion (toutes orphelines, retirées) : `_straighten_btn_action()` (routage bi-mode du clic gauche), `_toggle_straighten_mode()` (clic droit), `_straighten_selected_image()` (n'était appelée que par `_straighten_btn_action` en mode manuel). Le callback `"straighten"` de la colonne d'icônes (`icon_toolbar_qt.py`) pointe désormais directement vers le même callback que `deskew_selected` du menu.

**`state.straighten_mode` SURVIT à ce nettoyage** (exception explicite d'`idees.txt`) : il reste utilisé par l'icône bi-mode straighten **interne à la barre d'outils de la visionneuse** (voir section dédiée plus bas, ce mécanisme-là n'a pas changé), et `PanelWidget._set_straighten_mode()` (persistance appelée depuis cette barre) reste en place. Seul le bi-mode de la colonne d'icônes a disparu — la persistance/le state partagé restent.

**Activation** : `has_selected_images()` (`_ACTIVATION_RULES["straighten"]`, `icon_toolbar_qt.py`) — inchangé.
...
**Depuis le 2026-08-14, `PanelWidget._straighten_selected_image()` n'existe plus** (méthode orpheline retirée avec le nettoyage du bi-mode de la colonne d'icônes — voir plus bas). Le mode manuel n'a plus de point d'entrée dédié qui ouvrirait la visionneuse directement sur l'outil straighten : on l'atteint en ouvrant la visionneuse normalement (double-clic sur une vignette) puis en sélectionnant l'icône `straighten` dans sa barre d'outils. Il n'y a donc plus de logique de secours "première image sélectionnée / première image de la mosaïque" à documenter ici — c'est simplement la page affichée au moment où l'outil est activé. La navigation entre pages reste celle, commune, de la visionneuse principale (flèches/molette), sur toutes les pages de la mosaïque (y compris non-images, simplement ignorées par `get_image_indices()`).
...
## Icône Redressage de la barre d'outils — bi-mode manuel/auto (v1.7.3+, seul bi-mode restant depuis le 2026-08-14)

L'icône `"straighten"` de `_ViewerToolbar` (`modules/qt/viewer_toolbar_qt.py`) n'est pas un simple `_ToolButton` mono-fonction : elle pilote `state.straighten_mode` (0=manuel/1=auto) en interne à la visionneuse. **C'est désormais le seul bi-mode straighten de l'application** — la colonne d'icônes, elle, est mono-fonction (voir plus haut) et ne fait plus que déclencher l'automatique en lot, sans lire ni écrire `state.straighten_mode` autrement que pour rester synchronisée en affichage (tooltip fixe, pas de bascule).
...
- **Tooltip dynamique** (`_ViewerToolbar._update_straighten_tooltip`) : préfixe systématiquement `<b>Redressage</b>` (clé `viewer.toolbar_straighten_tooltip`) en gras, suivi de `tooltip.straighten_mode_0`/`straighten_mode_1` sur une seconde ligne — sans ce préfixe, rien n'indique quelle fonction appelle l'icône tant qu'on n'a pas cliqué (piège identifié et corrigé pendant l'implémentation). Recalculé à chaque `retranslate()` (construction, changement de langue/thème) et à chaque bascule de mode.
- **Callback `set_straighten_mode`** (`PanelWidget._set_straighten_mode`, `panel_widget.py`, ajouté aux dict retourné par `_image_viewer_callbacks()`) : persiste le mode sans le re-basculer (déjà fait côté `_ViewerToolbar` avant l'appel) — `_renumber_config().set_straighten_mode(mode)` + `_refresh_toolbar_states()`. C'est la seule méthode de persistance restante pour `state.straighten_mode` depuis la suppression de `PanelWidget._toggle_straighten_mode()` (colonne d'icônes, 2026-08-14).
...
3. **Colonne d'icônes** — bouton `straighten`, mono-fonction depuis le 2026-08-14 (voir section dédiée plus haut), pointe directement vers le même callback `deskew_selected`.

Callbacks (`PanelWidget._deskew_callbacks()`, inchangé) : `parent`, `save_state`, `render_mosaic`, `update_button_text`, `refresh_status`, `canvas`, `state`, `rollback` — même forme que `_image_transforms_callbacks()` du skill `rotate-flip` (worker + overlay + annulation), plus `parent` pour le centrage de la fenêtre de résumé.

**Manuel (unitaire) — un seul point d'entrée** : l'icône bi-mode `straighten` **de la barre d'outils de la visionneuse principale** (section "Icône Redressage de la barre d'outils" plus haut), une fois la visionneuse déjà ouverte par un autre moyen (double-clic sur une vignette). Il n'existe plus de point d'entrée dédié depuis la mosaïque ou les menus qui ouvrirait directement la visionneuse sur cet outil — `context_menu.image.straighten_manual`, la clé de callback `"show_straighten_viewer"` et la méthode `PanelWidget._straighten_selected_image()` qui la portait ont tous été retirés le 2026-08-14 (nettoyage des commandes redondantes, `idees.txt` #3).

## Traductions

`locales/fr.json` (et 45 autres langues, dont les 3 variantes CSUR régénérées par script) :
- `context_menu.image.straighten_auto` (l'ancienne clé sœur `straighten_manual` a été supprimée le 2026-08-14 avec le point d'entrée qu'elle nommait).
- `tooltip.straighten` (nom du bouton mono-fonction de la colonne d'icônes/fenêtre de config des icônes ; reformulé le 2026-08-14 en "Redresser l'image automatiquement" — retraduit individuellement dans chaque langue, pas recopié depuis `tooltip.straighten_auto`) + `tooltip.straighten_mode_0`/`straighten_mode_1` (tooltip dynamique au survol de l'icône bi-mode **de la barre d'outils de la visionneuse uniquement**, selon le mode courant — n'est plus utilisée par la colonne d'icônes depuis que celle-ci est mono-fonction).
