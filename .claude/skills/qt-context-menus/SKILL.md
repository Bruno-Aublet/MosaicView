...
- **`menu.deleteLater()` obligatoire après `menu.exec(...)` pour tout menu dont une `QAction` porte `setShortcut(...)`.** Dans `context_menus_qt.py`, `_make_menu(parent)` crée le `QMenu` avec le panneau comme parent Qt permanent ; sans destruction explicite après fermeture, chaque ouverture du menu (clic droit) laisse un `QMenu` fantôme vivant, avec ses propres `QAction` à raccourci (Ctrl+Z/Ctrl+Y/Ctrl+V/Ctrl+I/Esc dans `show_canvas_context_menu`/`show_image_context_menu`) toujours actives. À force d'ouvertures, ces raccourcis fantômes entrent en conflit avec le `QShortcut` global équivalent (câblé dans `MosaicView.py`) — Qt désactive alors silencieusement le raccourci ambigu (aucun message, la touche ne fait plus rien), alors que cliquer l'item du menu continue de fonctionner puisqu'il invoque directement le callback sans passer par la résolution de raccourci. Les trois fonctions de `context_menus_qt.py` (`show_canvas_context_menu`, `show_image_context_menu`, `show_dir_context_menu`) appellent `menu.deleteLater()` juste après `menu.exec(global_pos)` pour cette raison — tout nouveau menu contextuel de ce fichier (ou tout `QMenu` ailleurs qui poserait un `setShortcut` déjà utilisé comme raccourci global) doit faire de même.
...
**Piège — ne pas mettre en italique.** `font-style: italic` (en CSS ou via `act.setFont()`) sur un item désactivé casse l'alignement de tout le menu : Qt calcule l'indentation/la largeur de chaque `QAction` selon sa propre police, donc un item avec une police différente des autres (italique vs normale) décale visuellement tout le menu (icônes, raccourcis clavier, flèches de sous-menu désalignés). La couleur grisée seule suffit à distinguer un item désactivé — ne pas ajouter l'italique.
