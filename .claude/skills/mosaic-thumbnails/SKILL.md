...
- **Piège corrigé (2026-08-14) — `_tw()`/`_th()`/`_cw()`/`_ch()` lisaient un singleton global partagé entre panel1/panel2, pas le state du panneau réellement concerné** : `modules.qt.state.state` est un pointeur unique, réassigné manuellement ("bascule" push/pop) vers le state du panneau à traiter juste avant chaque opération — correct uniquement de façon synchrone, jamais dans un callback Qt asynchrone (resizeEvent, QTimer.singleShot...). Scénario reproduit : en split-view, sélectionner une vignette dans le panneau chargé puis ouvrir le dropdown de langue (footer de la colonne d'icônes) déclenchait un `resizeEvent` du canvas à un moment où le singleton pointait déjà sur l'autre panneau (vide) — `boundingRect()`/`_relayout()`/`render_mosaic()` utilisaient alors sa taille de vignette (`thumb_w`/`thumb_h`) au lieu de celle du panneau affiché : cadre de sélection à la mauvaise taille, grille repositionnée n'importe où. Fix : `_tw()`/`_th()`/`_cw()`/`_ch()` acceptent désormais un paramètre `scene=None` ; une fonction `_resolve_state(scene)` (ligne ~160) retrouve le state via `scene.views()[0]._state` (le `MosaicCanvas` propriétaire de la scène) en priorité, fallback sur le singleton global seulement si `scene` est `None` ou n'a pas encore de vue attachée. `ThumbnailItem`/`DirItem` stockent tous deux `self._scene` (passé au constructeur) et l'utilisent systématiquement dans tous leurs appels internes à ces 4 fonctions, ainsi que `MosaicCanvas` via `self._scene`. **Toute nouvelle méthode de `ThumbnailItem`/`DirItem`/`MosaicCanvas` qui a besoin de `_tw()`/`_th()`/`_cw()`/`_ch()` doit leur passer `self._scene`**, ne jamais les appeler sans argument dans ce fichier. `DirItem` a gagné un paramètre `scene` à son constructeur à cette occasion (absent avant ce fix).
...
- **Piège corrigé (2026-08-09) — le fallback PIL de `_get_pixmap_for_size` doit toujours construire à `THUMB_SIZES[2]`, jamais à `(tw, th)`** : ce fallback se déclenche quand `qt_qimage_large` n'est pas encore prêt, typiquement pendant le chargement d'une archive, avant que `build_qimage_for_entry` n'ait traité l'entrée dans le thread background — un changement de taille de vignette pendant ce court intervalle appelait autrefois `_get_pixmap_for_size` avec la taille demandée à cet instant (ex. petite, 100×133), et le fallback construisait `qt_pixmap_large` directement à cette taille au lieu de 200×267. Ce cache basse résolution restait ensuite figé ; tout agrandissement ultérieur upscalait ce pixmap déjà réduit → vignettes durablement floues pour les entrées concernées, même après la fin du chargement. Reproductible en ouvrant un comic par-dessus un comic déjà chargé et en changeant la taille de vignette pendant le second chargement. Fix : le fallback construit systématiquement à `THUMB_SIZES[2]` (comme `build_qimage_for_entry`), puis laisse l'étape 2 scaler vers `(tw, th)` — même garantie de résolution de référence que le chemin normal, quel que soit le moment où le fallback se déclenche.
...
- **Piège corrigé (2026-08-13) — le damier de transparence débordait hors de la forme réelle de l'image** : `create_centered_thumbnail()` peignait autrefois le damier sur tout le cadre `thumb_w × thumb_h` avant d'y coller l'image — une image dont le ratio ne correspond pas à celui du cadre (ex. une icône carrée dans un cadre 150×200) affichait alors le damier en excès dans le padding autour de l'image, alors que cette zone n'est pas transparente dans le fichier source, juste hors de son cadre. Fix : le damier est désormais peint sans masque uniquement sur le rectangle de `img_thumb` (l'image déjà redimensionnée), puis l'image est collée par-dessus avec son propre alpha comme masque. Piège rencontré en cours de fix : coller le damier directement avec `img_thumb` comme masque (`paste(checker, pos, img_thumb)`) fait apparaître le damier là où l'image est **opaque** et rien là où elle est transparente — l'inverse de l'effet recherché, PIL ne peignant le pixel du dessus que là où le masque a un alpha non nul.
- **Piège corrigé (2026-08-14) — la taille de vignette sauvegardée était systématiquement écrasée à `'normal'` au redémarrage, ET les deux panneaux partageaient la même clé de config** : deux bugs distincts, découverts et corrigés ensemble.
  1. Dans `ThumbSizeSlider.__init__` (`icon_toolbar_qt.py`), `self._slider.valueChanged.connect(self._emit_change)` était câblé **avant** `self._slider.setValue(initial_index)`. Un `QSlider` neuf démarre à sa valeur minimale (0) ; comme `IconToolbar` construit toujours la réglette avec `initial_index=1` en dur (`self._thumb_size_index = 1` par défaut, avant même que `_init_thumb_size()` n'ait lu la vraie valeur sauvegardée), le `setValue(1)` déclenchait un `valueChanged(1)` prématuré qui remontait jusqu'à `PanelWidget._apply_thumb_size(1, save=True)` — écrasant silencieusement la config avec `'normal'` juste avant que `_init_thumb_size()` (appelé juste après dans `_build_icon_toolbar()`) ne relise (déjà écrasée) et applique cette même mauvaise valeur. Toute taille non-"moyenne" était donc effacée à chaque démarrage. Fix : `setValue(initial_index)` est appelé avant de connecter `valueChanged`.
  2. Une fois (1) corrigé, le partage d'une seule clé de config `thumbnail_size` entre panel1 et panel2 est devenu visible : changer la taille dans un seul panneau redéfinissait aussi la taille de l'autre au redémarrage. Fix : voir section persistance plus haut (clé `thumbnail_size_panel2` + `Panel2Config`) — ce n'était pas une régression de (1), c'est le design d'origine (présent depuis la v1.0.0) qui n'avait simplement jamais été remarqué tant que (1) empêchait toute sauvegarde de tailles différentes de `'normal'`.
