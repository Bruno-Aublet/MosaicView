...
description: Localiser ou modifier la conversion par lot PDF→CBZ de MosaicView (process PyMuPDF préchauffé, détection DPI, owner-password). Utiliser dès qu'une tâche touche à batch_convert_pdf_to_cbz ou au traitement "Conversion en lot de fichiers PDF en CBZ".
---

# Conversion par lot PDF→CBZ — MosaicView

Un des 8 traitements par lot du projet (skill `batch-processing`, **à lire en premier** pour l'architecture commune : pattern confirm/progress/summary, contrat `batch_callbacks`, deux points d'entrée menu/drop). **Le flux le plus structurellement différent des 4 conversions classiques** — pas de `rarfile`/`7z.exe`/`tarfile` en lecture directe, mais un **process Python séparé et préchauffé** dédié au décodage PyMuPDF, communiquant par messages. Dans `modules/qt/batch_dialogs_qt.py:1774-2033`, protocole implémenté dans `modules/qt/pdf_loading_qt.py`.
...
## Architecture — process séparé préchauffé, pas un thread classique

**Le point le plus important à comprendre avant de toucher à ce flux.** `_run_pdf_conversion` lance bien un `threading.Thread` (`do_conversion`) comme les 3 autres flux classiques — mais ce thread ne décode **aucun** PDF lui-même : il envoie des messages à un **process séparé** (`multiprocessing`, pas `threading`) via une file (`_merge_in_q`) et reçoit les réponses via un pipe (`_merge_out_conn`), tous deux définis dans `modules/qt/pdf_loading_qt.py` et **partagés avec le chargement PDF normal** (hors batch — voir skill `viewers`, mémoire `project_pdf_support.md` pour l'architecture complète, préchauffage au démarrage de l'app compris).

- **`_ensure_merge_process()`** (`pdf_loading_qt.py:857`) : démarre le process (`multiprocessing.get_context('spawn').Process`, cible `_pdf_persistent_process`, `daemon=True`) s'il n'existe pas déjà ou est mort — **réutilisé d'un fichier à l'autre du même lot**, pas relancé à chaque PDF. Le process reste actif tant que l'application tourne (ou tant qu'il n'a pas été explicitement arrêté), d'où "préchauffé" : le coût de démarrage d'un interpréteur Python + import de PyMuPDF n'est payé qu'une fois pour tout le lot (et potentiellement réutilisé au-delà, pour un chargement PDF normal ultérieur).
- **`_send(msg)`/`_recv(timeout)`** : wrappers locaux à `do_conversion` autour de `_merge_in_q.put(msg)`/`_merge_out_conn.poll(timeout) + recv()` — chaque réception a un timeout explicite (30s pour l'ouverture, 120s pour une page) ; un timeout est traité comme une erreur pour ce fichier, pas un blocage indéfini de tout le lot.
...
- **Changer la logique de détection DPI** : uniquement dans `pdf_loading_qt.py`, bloc `elif kind == 'batch_convert':` — **partagé** avec le chargement PDF normal (voir skill `viewers`), toute modification doit être revalidée sur les deux usages.
...
- **Le process est partagé avec le chargement PDF normal** — ne pas supposer qu'il est dédié au batch ; une modification du protocole de messages doit être testée sur les deux chemins d'usage.
...
- `pdf-loading` — le chargement PDF **normal** (hors batch) partage le même process préchauffé et le même protocole de messages (`preopen`/`run`/`run_merge`/`batch_open`/`batch_convert`) ; voir ce skill pour l'architecture complète (préchauffage au démarrage, détection DPI, déverrouillage owner-password).
