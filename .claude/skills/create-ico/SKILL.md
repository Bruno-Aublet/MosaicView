...
Une fois inséré dans la mosaïque, un `.ico` reste une entrée `is_image: True` comme les autres et peut subir les mêmes opérations (rotation, crop, ajustements...) — **`entries.py::save_image_to_bytes()` gère nativement le cas `.ico`** (ajouté le 2026-08-16, bug découvert à l'occasion de la migration de l'outil transparence dans la barre d'outils de la visionneuse) : elle relit `info['sizes']` du fichier `.ico` d'origine (`entry['bytes']`, un `set` de tuples `(w, h)` exposé par PIL) et régénère exactement ce même jeu de résolutions à la sauvegarde (`save(format='ICO', sizes=list(original_sizes))`) — ni plus ni moins de tailles que le fichier n'en avait avant modification, qu'il soit mono ou multi-résolution. **Avant ce correctif, `save_image_to_bytes()` n'avait aucun cas `.ico` explicite** et un tel fichier retombait silencieusement dans son `else` générique (sauvegarde en PNG), perdant son format multi-résolution — ce bug touchait **tous** les outils qui appellent cette fonction générique (crop, straighten, clone, texte, formes, deskew, adjustments classiques), pas seulement un seul point d'entrée. L'ancien code de `AdjustmentViewerDialog._apply_transparency()` (fichier `adjustments_viewers_qt.py`, supprimé) évitait ce piège localement en forçant `fmt = 'ICO'` avant un `save()` direct (sans `sizes=`, donc une seule résolution) — ce garde-fou local a disparu avec le fichier, remplacé par la correction centralisée ci-dessus, plus complète (fidèle au jeu de tailles réel) et plus large (tous les appelants).
...
- `viewers`/`adjust-transparency` — l'outil transparence de la barre d'outils de la visionneuse, dont la migration a révélé le bug de `save_image_to_bytes()` corrigé ci-dessus ; tout outil de cette barre appliqué à une entrée `.ico` en bénéficie désormais.
- `apply-image-operation` — `save_image_to_bytes()` (`entries.py`), le point de passage générique de sauvegarde après modification de `entry['img']`, désormais correct pour `.ico` ; référencé par la quasi-totalité des outils d'édition d'image du projet.
