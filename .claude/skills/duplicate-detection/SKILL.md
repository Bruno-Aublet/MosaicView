...
C'est déjà fait dans tous les points d'écriture existants (`image_ops.py`, `image_transforms_qt.py`, `image_processing_qt.py`, `crop_tool_qt.py`, `straighten_tool_qt.py`, `clone_tool_qt.py`, `text_tool_qt.py`, `shapes_tool_qt.py`, `transparency_tool_qt.py`, `color_depth_tool_qt.py`, `effects_tool_qt.py`, `image_mode_tool_qt.py`, `resize_dialog_qt.py`, `image_viewer_qt.py`, `file_operations_qt.py`, `undo_redo_qt.py`, `panel_widget.py`) — voir skill `apply-image-operation` pour le pattern général de modification d'image, qui inclut cette invalidation. **Si tu écris une nouvelle fonction qui touche à `entry["bytes"]`, ajoute `entry["_hash"] = None` dedans**, sinon la détection de doublons devient silencieusement fausse pour cette page.
