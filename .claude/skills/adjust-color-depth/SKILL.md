...
- **Icône de la barre** : `BTN_Color_Depth.png`, `tool_id="color_depth"` dans `viewer_toolbar_qt.py::_ViewerToolbar.__init__` — pas de bi-mode, pas de grisage conditionnel selon le format (contrairement à compression/transparency : la profondeur de couleur s'applique quel que soit le format source)
...
Logique PIL strictement inchangée par la migration :
...
**Dépendance à `original_ext`** : le comportement de `'8'` change selon le format source — `apply_image_adjustments()` (`image_processing_qt.py`) injecte automatiquement `settings['original_ext']` depuis `entry['extension']`, `color_depth_tool_qt.py::perform_color_depth()` n'a pas besoin de le passer explicitement (contrairement à l'ancien dialog, qui le stockait lui-même dans `self._original_ext`). Ne pas court-circuiter ce paramètre en testant le mode PIL actuel de l'image — c'est bien l'extension de fichier d'origine qui pilote la branche, pas le mode PIL courant.

**`for_preview`** : n'a plus d'usage réel côté barre d'outils (pas de preview live pour cet outil, chaque clic commit directement avec `for_preview` implicite `False`) — reste utilisé par l'ancien mécanisme de preview 300×300, désormais mort pour cette section puisque le panneau Ajustements classique ne l'appelle plus jamais avec `color_depth` renseigné.
...
- `adjust-image-mode` — l'autre réglage de mode, appliqué juste avant celui-ci dans le pipeline, migré dans la barre d'outils selon le même modèle.
- `apply-image-operation` — pattern undo/redo respecté par `apply_image_adjustments()`.
- `qt-tooltips` — `OverlayTooltip`, tooltip de l'icône `color_depth` de la barre.
