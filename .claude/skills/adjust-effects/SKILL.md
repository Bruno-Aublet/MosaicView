...
- **Traitement PIL** : `image_processing_qt.py::apply_adjustments()`, bloc `# ── Effets ──`
...
`'none'` reste une valeur par défaut acceptée par `apply_adjustments()` (aucun traitement, no-op) mais n'a pas de radio équivalent côté barre d'outils, voir écart ci-dessus.
...
- `adjust-image-mode` — l'autre réglage de mode, cadré lui plus fidèlement sur color_depth (verrouillage dérivé du mode PIL réel).
