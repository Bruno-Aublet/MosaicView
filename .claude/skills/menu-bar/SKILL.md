...
| `_populate_images_menu` | Images | Sous-menu Rotation (skill `rotate-flip`), redimensionner (`page-resize`), ajustements (`adjustments-panel`), redressement **automatique uniquement** (`page-straighten` — le manuel, comme le recadrage/clonage/texte/netteté, n'a plus d'entrée ici depuis leur migration dans la barre d'outils de la visionneuse principale, voir skill `viewers`), conversion de format (`image-format-conversion`), join/split (`page-merge`/`page-split`), GIF animé (`animated-gif`), créer .ico (`create-ico`), remplacer/supprimer image corrompue (`corrupted-images`) |
