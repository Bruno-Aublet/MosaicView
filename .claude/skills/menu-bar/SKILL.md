...
**Depuis le 2026-08-14** : les 5 fonctions qui ont besoin du state (`_populate_file_menu`, `_populate_edit_menu`, `_populate_images_menu`, `_populate_archives_menu`, `_populate_metadata_menu`) lisent `st = callbacks.get('state') or _state_module.state`, **pas** `_state_module.state` directement. `callbacks['state']` est injecté par `PanelWidget._build_menubar_callbacks()` (`panel_widget.py`) — c'est le `self._state` du panneau propriétaire du menu, capturé au moment de la construction du dict de callbacks. Raison : `_state_module.state` est un singleton global partagé entre panel1/panel2 (voir piège daté 2026-08-14 dans le skill `mosaic-thumbnails`) — un menu ouvert sur `aboutToShow`, signal purement piloté par le clic utilisateur, n'a aucune garantie que ce singleton pointe encore sur le bon panneau au moment où il s'ouvre. Le fallback sur le global couvre uniquement les rares appelants de `_populate_*_menu` hors du flux normal `build_menubar(panel, panel._build_menubar_callbacks(), ...)`. **Toute nouvelle fonction `_populate_xxx_menu` qui lit le state doit suivre ce même pattern**, jamais relire `_state_module.state` directement.
...
| `_populate_images_menu` | Images | Sous-menu Rotation (skill `rotate-flip`), redimensionner (`page-resize`), redressement **automatique uniquement** (`page-straighten` — le manuel, comme le recadrage/clonage/texte/netteté/effets/mode d'image/profondeur de couleur, n'a plus d'entrée ici depuis leur migration dans la barre d'outils de la visionneuse principale, voir skill `viewers`), conversion de format (`image-format-conversion`), join/split (`page-merge`/`page-split`), GIF animé (`animated-gif`), créer .ico (`create-ico`), remplacer/supprimer image corrompue (`corrupted-images`) — l'entrée "Ajustements d'image" a été retirée le 2026-08-16 en même temps que la fenêtre `AdjustmentsDialog` elle-même, une fois ses 3 dernières fonctions migrées dans la barre d'outils |
...
- `mosaic-thumbnails` — piège daté 2026-08-14 sur le singleton `_state_module.state` partagé entre panneaux, à l'origine du pattern `callbacks.get('state')` décrit plus haut.
