...
- `_IconConfigDialog` est non-modale, titre via `_wt()`. Centrage : `setWindowFlags(Qt.Window)` (obligatoire, sinon `move()` avant `show()` est sans effet — absent = régression silencieuse) + `position_dialog_on_parent(dlg, panel)` appelé dans `_open_config_window` **avant** `dlg.show()`, pattern standard sans flash (voir `dialogs_qt.py::position_dialog_on_parent`). Le `panel` de centrage doit être le `PanelWidget` complet, jamais `self` (l'`IconToolbarQt`, large de seulement ~130-260px selon le palier d'icône) : centrer un dialogue de 1100px sur cette colonne étroite pousse son `x` très négatif, clampé à 0 par le clamp écran de `_center_on_widget` → dialogue plaqué au bord gauche au lieu d'être centré. `_open_config_window` (ligne ~2042) remonte donc la hiérarchie Qt (`parentWidget()` en boucle) jusqu'à trouver l'instance `PanelWidget` avant d'appeler `position_dialog_on_parent`.
