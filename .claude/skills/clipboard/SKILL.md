...
## Emplacement des dossiers `clipboard_*`

`PanelWidget._get_temp_dir()` ([panel_widget.py:747](../../../modules/qt/panel_widget.py#L747)) — la fonction injectée comme `get_temp_dir_func` dans `copy_to_system_clipboard`/`cut_selected` — appelle `get_mosaicview_temp_dir()` (`temp_files.py`), donc pointe vers `%TEMP%\MosaicViewTemp\`. C'est important pour que les dossiers `clipboard_*` créés par un copier/couper de pages restent dans le périmètre des deux mécanismes de nettoyage (`cleanup_all_temp_files()` rétention 12h, et le bouton "Effacer le presse-papiers"), qui ne scannent que ce dossier — ne jamais faire pointer `get_temp_dir_func` ailleurs (ex. la racine `%TEMP%`), sous peine de dossiers `clipboard_*` qui s'accumulent hors de portée de tout nettoyage. Ce même callback est aussi injecté sous le nom `"get_mosaicview_temp_dir"` dans les dialogues batch ([panel_widget.py:687](../../../modules/qt/panel_widget.py#L687) et [panel_widget.py:789](../../../modules/qt/panel_widget.py#L789), voir skill `batch-processing`).

`cleanup_legacy_root_clipboard_dirs()` (`temp_files.py`, voir skill `temp-files`) est appelée une fois au lancement dans `MosaicView.py::main()` juste après `cleanup_stale_mei_dirs()` : elle balaie la racine `%TEMP%` et supprime tout dossier `clipboard_*` qui s'y trouve, sans condition d'âge (leur seule présence à cet endroit suffit à les qualifier d'orphelins — un `clipboard_*` ne doit jamais légitimement se trouver ailleurs que dans `MosaicViewTemp`).
...
- **`qt-context-menus`** — si Ctrl+V (ou Ctrl+Z/Ctrl+Y/Ctrl+I) semble ne plus rien faire alors que l'action équivalente du menu clic-droit fonctionne toujours, la cause n'est pas ce module mais un raccourci `QAction` fantôme accumulé par un `QMenu` non détruit (voir section "Pièges" de ce skill) — ne pas chercher le bug côté presse-papiers Windows dans ce cas.
