"""
modules/qt/saturation_tool_qt.py — Outil "saturation" de la barre d'outils
flottante de la visionneuse principale (image_viewer_qt.py).

Fusion progressive des visionneuses (idees.txt #3, 7e outil migré, 4e des 8
modes d'ajustement après sharpness/unsharp/brightness) : ce module contient
toute la logique propre à l'outil "saturation" — état + preview live (mixin
SaturationCanvasMixin, hérité par _ViewerCanvas), commit de l'ajustement dans
l'historique du panneau (mixin SaturationViewerMixin, hérité par ImageViewer),
et le panneau flottant de la réglette (_SaturationOptionsPanel).
...
PENDANT le déplacement du slider (valueChanged) ; au relâchement du clic
(sliderReleased), l'ajustement est commité automatiquement dans
entry['bytes'] (perform_saturation) et devient sa propre entrée d'historique
...
ImageViewer._has_unvalidated_work(), pas de _saturation_by_page, pas de
...
# ─────────────────────────────────────────────────────────────────────────────
# Panneau flottant de la réglette de saturation
# ─────────────────────────────────────────────────────────────────────────────

class _SaturationOptionsPanel(QWidget):
    """Panneau flottant avec la réglette de saturation (-100..+100), affiché
    sous la barre d'outils uniquement quand l'outil "saturation" est actif —
    même principe que _SharpnessOptionsPanel (jamais inséré dans le layout de
    ImageViewer, indépendant du timer d'auto-masquage de la barre pour ne pas
    interrompre un réglage en cours).
    ...
    réel via SaturationViewerMixin.perform_saturation() au relâchement du
    ...
    def _apply_theme(self):
        ...
        self.setStyleSheet(floating_options_panel_style(theme, "_SaturationOptionsPanel"))
    ...
    def retranslate(self):
        ...
        self._label.setText(_("viewer.saturation_panel_label"))
    ...
    def _on_slider_changed(self, value: int):
        ...
        if self._spin.value() != value:
            ...
            self._spin.setValue(value)
            self._spin.blockSignals(False)
        self._viewer._update_saturation_preview()
    ...
    def _on_slider_released(self):
        self._viewer.perform_saturation()
...
class SaturationCanvasMixin:
    """Hérité par _ViewerCanvas (image_viewer_qt.py) en plus de QLabel : ajoute
    l'état minimal de l'outil "saturation" au canvas de la visionneuse, sans
...
class SaturationViewerMixin:
    """Hérité par ImageViewer (image_viewer_qt.py) en plus de QDialog : ajoute
    la logique de l'outil "saturation" au viewer, sans que son code vive dans
    image_viewer_qt.py. Suppose que l'hôte a déjà self._canvas
    (_ViewerCanvas, avec SaturationCanvasMixin), self.callbacks,
    self.current_idx, self._toolbar (avec _saturation_panel), et
    self._sharpness_preview_img (champ de preview PARTAGÉ avec sharpness/
    unsharp/brightness, défini dans image_viewer_qt.py::ImageViewer.__init__
    ...
    def perform_saturation(self):
        ...
        que perform_brightness(), corrigé le 2026-08-14 — l'ancien
        comportement "retour à 0" était une erreur, jamais demandée) : il
        reste sur la valeur qui vient d'être appliquée, pour rester une
        indication visuelle de l'ajustement en cours plutôt que de donner
        l'impression que le réglage a été perdu alors que l'image a bien
        changé. Un nouveau geste après ce commit applique un ajustement
        ...
        try:
            ...
            state.saturation_value_by_history_index[(self.current_idx, state.history_index)] = value
    ...
    def _reset_saturation_preview(self):
        ...
        _reset_brightness_preview() (corrigé le 2026-08-14 pour aligner
        saturation sur brightness plutôt que sharpness — le retour à 0
        était une erreur, jamais demandée).

        Il ne peut jamais y avoir de valeur "en attente" à committer (le
        relâchement du slider commit déjà tout, voir perform_saturation),
        mais le slider doit refléter la dernière valeur RÉELLEMENT commitée
        sur la page affichée, pas systématiquement 0 : state.
        saturation_value_by_history_index retrouve cette valeur si un commit
        saturation existe pour (page, history_index) courants, sinon 0 (page
        jamais ajustée à ce point d'historique)."""
        from modules.qt import state as _state_module
        self._sharpness_preview_img = None
        state = self.callbacks.get('state') or _state_module.state
        value = state.saturation_value_by_history_index.get(
