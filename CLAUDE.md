...
- **Fusion des visionneuses (idees.txt #3, chantier TERMINÉ le 2026-08-16) — NE JAMAIS migrer le code d'un outil dans `image_viewer_qt.py`.** Les 16 outils migrés (crop, redressage, clonage, texte, netteté/netteté adaptative, luminosité-contraste, saturation, suppression des couleurs, compression, niveaux, transparence, profondeur de couleur, effets, mode d'image) et les outils neufs ajoutés depuis (formes, rotation — aucun des deux n'est une migration) ont chacun leur logique dans leur propre module séparé (ex. `crop_tool_qt.py`, `straighten_tool_qt.py`, `clone_tool_qt.py`, `transparency_tool_qt.py`, `color_depth_tool_qt.py`, `effects_tool_qt.py`, `image_mode_tool_qt.py`) — `image_viewer_qt.py` orchestre/appelle ces modules (état minimal de branchement, délégation), mais ne contient JAMAIS leur implémentation (calculs, gestion des événements souris propres à l'outil, rendu de l'overlay, panneaux flottants d'options). **Why** : consigne expresse de l'utilisateur donnée dès le début de ce chantier — sans cette séparation, `image_viewer_qt.py` grossit à chaque outil migré et devient ingérable une fois tous les outils fusionnés. **Règle absolue, sans exception, qui continue de s'appliquer à tout nouvel outil ajouté à cette barre** : avant de toucher à un outil existant ou d'en ajouter un nouveau, vérifier explicitement que son code vit bien dans son propre module et pas dans `image_viewer_qt.py` — si ce n'est pas le cas (dette existante), le signaler et proposer une extraction avant de continuer, ne jamais l'aggraver en ajoutant davantage de code d'outil dans ce fichier. La visionneuse annexe d'ajustements plein écran (`AdjustmentViewerDialog`/`adjustments_viewers_qt.py`) et le panneau non-plein-écran `AdjustmentsDialog`/`adjustments_dialog_qt.py` (une fois ses 3 dernières fonctions — profondeur de couleur, effets, mode d'image — migrées à leur tour) ont tous deux été intégralement supprimés (2026-08-16) — il n'existe plus qu'une seule fenêtre dans toute l'application pour la lecture ET l'édition d'image, voir skill `viewers` pour le détail.
