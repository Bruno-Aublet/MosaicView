...
- **NE JAMAIS migrer le code d'un outil de la barre d'outils de la visionneuse principale dans `image_viewer_qt.py`.** Chaque outil a sa logique dans son propre module séparé (ex. `crop_tool_qt.py`, `straighten_tool_qt.py`, `clone_tool_qt.py`, `transparency_tool_qt.py`) — `image_viewer_qt.py` orchestre/appelle ces modules (état minimal de branchement, délégation) mais ne contient JAMAIS leur implémentation (calculs, événements souris propres à l'outil, rendu d'overlay, panneaux flottants d'options). **Why** : sans cette séparation, `image_viewer_qt.py` grossit à chaque outil et devient ingérable. **Règle absolue** : avant de toucher à un outil existant ou d'en ajouter un nouveau, vérifier que son code vit bien dans son propre module — si ce n'est pas le cas, le signaler et proposer une extraction avant de continuer. Il n'existe plus qu'une seule fenêtre dans toute l'application pour la lecture ET l'édition d'image (voir skill `viewers`).
- **Barre d'outils de la visionneuse principale — sélectionner n'importe quel outil force toujours le mode simple page, sans exception.** Implémenté en un seul point inconditionnel en tête de `_ViewerToolbar.set_active_tool` (`viewer_toolbar_qt.py`). **Why** : le mode double/continu affiche un pixmap combiné de 2 pages ; un outil qui calcule une position de clic ou une géométrie tracée par rapport à cette image combinée l'appliquerait silencieusement à une seule page. **Règle absolue** : ce forçage n'est jamais réservé à certains outils selon qu'ils semblent concernés ou non (y compris les outils sans geste souris sur le canvas) — s'applique automatiquement à tout nouvel outil de cette barre.
- **Boutons "Valider" ET "Annuler" flottants de la visionneuse principale (crop/straighten/text/shapes/transparency) — un seul mécanisme gère leur réapparition/disparition, pour les deux boutons identiquement.** Dans `image_viewer_qt.py` : `_update_validate_btn_state()`/`_update_cancel_btn_state()` recalculent texte/couleur/police/position mais n'appellent **jamais** `.show()`/`.hide()` (toujours les deux ensemble, jamais l'une sans l'autre) ; `_reveal_validate_btn()`/`_reveal_cancel_btn()`/`_conceal_validate_btn()`/`_conceal_cancel_btn()` sont les **seuls** points autorisés à afficher/masquer chaque bouton, appelés **uniquement** par `_ViewerToolbar`. **Why** : mélanger mise à jour d'état et affichage dans une même fonction appelée depuis de nombreux endroits désynchronise l'auto-masquage de la barre (bouton flottant seul, ou refusant de disparaître). **Règle absolue** : ne jamais appeler `.show()`/`.hide()` sur ces boutons en dehors de leurs `_reveal_*`/`_conceal_*`, ni ajouter un point d'appel hors `_ViewerToolbar`. Les deux boutons ont leur propre `enterEvent`/`leaveEvent` (suspend l'auto-masquage au survol, force le curseur normal, restent toujours `setEnabled(True)` pour continuer à recevoir ces événements) — toute nouvelle zone flottante de cette barre doit suivre ce même pattern.
- **Panneaux d'options flottants de la barre — blindage anti-fuite de clic obligatoire (`mousePressEvent`/`mouseReleaseEvent` avec `event.accept()`), sans exception, pour tout nouveau panneau.** **Why** : chaque panneau est un widget flottant enfant du canvas — sans ce blindage, un clic sur une zone vide (marges non absorbées par un widget enfant) fuit vers `_ViewerCanvas` en dessous et peut déclencher un geste d'outil involontaire (ex. un clic pipette rendant un pixel transparent).
- **Panneaux d'options flottants d'un outil à curseur custom sur le canvas — réinitialisation du curseur obligatoire au survol du panneau, sans exception.** **Why** : même cause que le blindage anti-fuite de clic ci-dessus — un panneau est un widget enfant du canvas, Qt ne réinitialise pas seul son curseur. **Fix** : `enterEvent` pose `setCursor(Qt.ArrowCursor)`, `leaveEvent` revérifie à 0ms (`QTimer.singleShot`) la position réelle du curseur avant `unsetCursor()` — voir skill `viewers` pour le détail et la liste des panneaux concernés. Ne s'applique qu'aux outils qui posent effectivement un curseur custom sur le canvas (pas les réglettes simples type brightness/saturation/compression).
